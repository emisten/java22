package se.iths.emilius.exercises.chapter8;
...
public class Streaming {
    ...
    public static void main(String[] args) {
        //    getListByPopulationSorted(getCountries(), 0);
        //    getListByPopulationSorted(getCountries(), 3);
    task19();
    ...
    private static void task19() {
        double sum = getCountries().stream()
                ...
                .mapToDouble(country -> country.population)
                .sum();
    ...
    private static void task18() {
        double sixSmallest = getCountries().stream()
                .sorted(Comparator.comparingInt(Country::area))
                .limit(6)
                .mapToDouble(Country::population)
                .sum();

        double threeLargest = getCountries().stream()
                .sorted(Comparator.comparingInt(Country::area).reversed())
                .limit(3)
                .mapToDouble(Country::population)
                .sum();

        System.out.println("The sum of the six smallest (by area) countries' population is: " + sixSmallest);
        System.out.println("The sum of the three greatest (by area) population is: " + threeLargest);
    }

    private static void task17() {
        getCountries().stream()
                .sorted(Comparator.comparing(Streaming::getReverse).reversed())
    ...
    private static void task16() {
        ...
        //      (country.countryName() + ", population: " + (int) (country.population() * 1_000_000)));
    ...
    record Statistics(String countryName, double density) {
        private static Statistics of(Country country) {
            return new Statistics(country.countryName(), country.population() * 1_000_000 / country.area());
        }
    }
    ...
    private static void task15() {
        var groupedByPopulation = getCountries().stream()
                ...
                .collect(Collectors.groupingBy(country -> Math.floor(country.population)));

        groupedByPopulation.entrySet().stream()
                .sorted(Map.Entry.comparingByKey())
                .forEach(Streaming::printCountryGroups);
    }

    private static void printCountryGroups(Map.Entry<Double, List<Country>> entrySet) {
        System.out.println("Länder med " + entrySet.getKey() + " miljoner invånare:");
    ...
    private static void task11() {
        getCountries().stream()
                .sorted(Comparator.comparingDouble(country -> country.population))
                .limit(5)
                .map(Country::countryName)
                .forEach(System.out::println);
    }
    ...
    private static void task10() {
        getCountries().stream()
                .filter(c -> c.countryName().length() > c.capitalCity().length())
                .map(Country::countryName)
                .forEach(System.out::println);
    }
    ...
    private static void task9() {
        getCountries().stream()
                .filter(country -> country.countryName().charAt(0) == country.capitalCity().charAt(0))
                .forEach(country -> System.out.println(country.countryName() + " " + country.capitalCity()));
        //.filter(country -> country.countryName.startsWith(String.valueOf(country.capitalCity.charAt(0))))
        // .forEach(country -> System.out.println(country.countryName() + ": " + country.capitalCity()));
    }
    ...
    private static void task6() {
        var averageArea = getCountries().stream()
                .mapToInt(Country::area)
                .average()
                .orElse(0);
        System.out.printf("Average area is: %.2f\n", averageArea);

        var nBelowAverage = getCountries().stream()
    ...
    record Country(String countryName, String capitalCity, double population, int area) {
    }
